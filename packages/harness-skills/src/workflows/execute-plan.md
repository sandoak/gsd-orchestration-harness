...
2. Use Task tool with subagent_type="harness-executor":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="harness-executor":
...
See `./.harness/skills/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ./.harness/skills/references/checkpoints.md for complete checkpoint guidance.
</step>
...
If you're reading this workflow because an orchestrator spawned you (vs running directly from /harness:execute-plan), the orchestrator's prompt will include checkpoint return instructions. Follow those instructions when you hit a checkpoint.
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `./.harness/skills/templates/user-setup.md`.
...
Use ./.harness/skills/templates/summary.md for structure.
...
For commit message conventions, see ./.harness/skills/references/git-integration.md
</step>
...
`/harness:execute-plan .planning/phases/{phase-dir}/{phase}-{next-plan}-PLAN.md`
...
- `/harness:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/harness:verify-work {Z}` — manual acceptance testing before continuing
- `/harness:discuss-phase {Z+1}` — gather context first
- `/harness:research-phase {Z+1}` — investigate unknowns
...
- `/harness:verify-work` — manual acceptance testing before completing milestone
- `/harness:add-phase <description>` — add another phase before completing
