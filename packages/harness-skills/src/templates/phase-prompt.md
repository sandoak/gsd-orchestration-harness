...
> **Note:** Planning methodology is in `agents/harness-planner.md`.
...
**Wave is pre-computed:** Wave numbers are assigned during `/harness:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `./.harness/skills/templates/user-setup.md` for full schema and examples
...
See `./.harness/skills/references/goal-backward.md` for derivation process.
See `./.harness/skills/workflows/verify-phase.md` for verification logic.
